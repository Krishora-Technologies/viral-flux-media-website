...
export function Inquiry() {
  ...
  // Phone choreography
  const phoneY = useTransform(p, [0, 0.45, 1], ["-55vh", "0vh", "0vh"]);
  const phoneRotateY = useTransform(p, [0, 0.45, 1], [540, 0, 0]); // 1.5 spins
  const phoneRotateZ = useTransform(p, [0, 0.45, 1], [-25, 0, 0]);
  const phoneScale = useTransform(p, [0, 0.2, 0.45, 1], [0.55, 0.7, 1, 1]);
  const phoneShadow = useTransform(
    p,
    [0, 0.45, 1],
    [
      "0 10px 30px -10px rgba(0,0,0,0.2)",
      "0 60px 120px -20px rgba(0,0,0,0.45), 0 0 0 1px rgba(0,0,0,0.04)",
      "0 60px 120px -20px rgba(0,0,0,0.45), 0 0 0 1px rgba(0,0,0,0.04)",
    ]
  );

  // Boot / app crossfade
  const lockOpacity = useTransform(p, [0.5, 0.65], [1, 0]);
  ...
  return (
    ...
    >
      ...
      <div className="pointer-events-none sticky top-0 h-screen w-full overflow-hidden">
        ...
        <div className="relative mx-auto grid h-full w-full max-w-[1500px] grid-cols-1 items-center gap-10 px-6 md:grid-cols-12 md:px-12">
          {/* Left column: heading */}
          <motion.div
          ...
          >
            <div className="font-mono-label mb-5 flex items-center gap-3 text-muted-foreground">
              <span className="h-px w-8 bg-ink/40" /> 02 — start
            </div>
          ...
          {/* Center: phone */}
          <div className="col-span-1 flex justify-center md:col-span-6" style={{ perspective: 1600 }}>
            <motion.div
              style={{
                y: phoneY,
              ...
              className="relative rounded-[48px] bg-ink p-3"
            >
          ...
          >
            <div className="space-y-3">
              ...
              <Chip label="min retainer" value="$8K/mo" />
...
function AppInterface() {
  ...
  const [form, setForm] = useState({ name: "", brand: "", goal: "growth", budget: "8-15k", note: "" });
  ...
  return (
    <>
      ...
      <div className="flex items-center justify-between px-5 pt-8">
        <div className="font-mono-label text-[9px] text-ink">9:41</div>
        <div className="font-mono-label text-[9px] text-ink">vfm · inquiry</div>
      </div>

      <div className="flex items-center justify-between px-5 pt-3">
        <div className="font-display text-xl italic leading-none">new brief</div>
      ...
      {!sent && (
        <div className="flex items-center justify-between gap-2 px-5 pb-6 pt-3">
          <button
            onClick={back}
            disabled={step === 0}
            className="rounded-full px-4 py-2 text-xs font-semibold text-ink/60 disabled:opacity-30"
          >
            ← back
          </button>
          {step < 2 ? (
            <button
              onClick={next}
              className="rounded-full bg-ink px-5 py-2.5 text-xs font-semibold text-cream transition-transform hover:scale-105"
            >
              continue →
            </button>
          ) : (
            <button
              onClick={submit}
              className="rounded-full bg-lime px-5 py-2.5 text-xs font-semibold text-ink transition-transform hover:scale-105"
            >
              send brief ↗
            </button>
          )}
